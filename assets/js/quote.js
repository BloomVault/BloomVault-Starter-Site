...
itemsEl.innerHTML = strains.map(s => `
  <div class="card">
    <h2>${s.name}</h2>
    <div class="form-row">
      <input class="input" type="number" min="0" step="1" placeholder="Qty" id="qty-${s.id}">
      <input class="input" placeholder="Notes (optional)" id="note-${s.id}">
    </div>
  </div>
`).join('');
